...
for i = 1 : numel(listing)
    ...
    y = single(y);
    label=single(label);
    save([mat_path, filename,'_cnn_filterbank_eeg.mat'], 'X', 'label', 'y', '-v7.3');
    clear X
    ...
    y = single(y);
    label=single(label);
    save([mat_path, filename,'_cnn_filterbank_eog.mat'], 'X', 'label', 'y', '-v7.3');
    clear X
    ...
    y = single(y);
    label=single(label);
    save([mat_path, filename,'_cnn_filterbank_emg.mat'], 'X', 'label', 'y', '-v7.3');
    clear X y label
end